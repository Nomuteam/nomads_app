...
export class CalendarPage {
    ...
    for (let key in a) {
      if (a[key].index == clave) {
        let aux2 = {
          ...
          'cancelation_policy': a[key].cancelation_policy == null ? 0 : a[key].cancelation_policy,
          'img': a[key].img,
          'cost': a[key].cost,
  ...
  ionViewWillEnter() {
    //this.EliminarClanSchedule('8ace2de5-d193-439b-b946');
    this.show_pop = true;
  ...
  doMath(evento) {
    ...
    if (evento.t_id != '') {
      ...
      //Sumamos uno a lugares
      this.EliminarClanSchedule(evento.index);
      
      //console.log('evento a salir del schedule:',evento);
    }
  ...
  EliminarClanSchedule(index) {
    let clanes = this.af.object('Users/' + firebase.auth().currentUser.uid + '/Clans/').snapshotChanges().subscribe(action => {
      this.clanes$ = action.payload.val();
      
      let u = this.clanes$;
      for (let key in u) {
        
        let schedule = this.af.object('Clans/' + u[key].index).snapshotChanges().subscribe(clanSchedule => {
          
          let scheduleAux = clanSchedule.payload.val();
          let i = scheduleAux.schedule;
          //console.log('schedule',i);
          for (let keyDetail in i){
            //console.log('buscamos para eliminar', i[keyDetail], keyDetail);
            if(i[keyDetail].index == index && i[keyDetail].nomad_index == firebase.auth().currentUser.uid){
              //console.log('Clans/'+u[key].index+'/schedule/'+keyDetail);
              this.af.list('Clans/'+u[key].index+'/schedule/'+keyDetail).remove();
            }
          }
  ...
  goAhead(evento) {
    ...
    console.log('Evento!!',evento, evento.index,evento.spaces_available);
    ...
    this.general_loader.present();
    //console.log('evento a salir del schedule2:',evento);
    ...
    //Actividad
    this.EliminarClanSchedule(evento.index);
    this.af.list('Events/').update(evento.index, {
      'spaces_available': evento.spaces_available + 1
    });
  }
